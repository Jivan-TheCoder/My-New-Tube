...
import com.playtube.protube.video.music.views.NewPipeRecyclerView;
...
public abstract class BaseListInfoFragment<I extends InfoItem, L extends ListInfo<I>>
    ...
    private void dynamicallyShowErrorPanelOrSnackbar(final ErrorInfo errorInfo) {
        if (infoListAdapter.getItemCount() == 0) {
            ...
            isLoading.set(false);
            // When some items are already visible, this is usually a non-fatal partial extraction
            // issue. Avoid showing a noisy snackbar and keep the current content visible.
            if (com.playtube.protube.video.music.BuildConfig.DEBUG) {
                Log.w(TAG, "Suppressed non-fatal list error with existing items: "
                        + errorInfo.getMessage(requireContext()));
            }
        }
    }
}
