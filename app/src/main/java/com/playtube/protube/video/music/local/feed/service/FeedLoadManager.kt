package com.playtube.protube.video.music.local.feed.service
...
import com.playtube.protube.video.music.ktx.getStringSafe
...
import com.playtube.protube.video.music.util.ChannelTabHelper
...
class FeedLoadManager(private val context: Context) {
     ...
     * [FeedGroupEntity.Companion.GROUP_ALL_ID], all subscriptions are loaded. When using
    ...
    fun startLoading(
        ...
        val outdatedSubscriptions = when (groupId) {
            FeedGroupEntity.Companion.GROUP_ALL_ID -> feedDatabaseManager.outdatedSubscriptions(
                outdatedThreshold
            )
    ...
    private fun loadStreams(
        ...
        try {
            ...
            if (originalInfo == null) {
                ...
                val channelInfo = ExtractorHelper.getChannelInfo(
                ...
                streams = channelInfo.tabs
                    ...
                    .map {
                        Pair(
                            ExtractorHelper.getChannelTab(subscriptionEntity.serviceId, it, true)
                                .onErrorReturn(storeOriginalErrorAndRethrow)
                                .blockingGet(),
                            it
                        )
                    ...
                    .flatMap { (channelTabInfo, linkHandler) ->
                        ...
                        if (channelTabInfo.relatedItems.isEmpty() &&
                            ...
                            val infoItemsPage = ExtractorHelper.getMoreChannelTabItems(
                                subscriptionEntity.serviceId,
                                linkHandler,
     ...
     * <br>
     * Remove streams from the feed which are older than [FeedDatabaseManager.Companion.FEED_OLDEST_ALLOWED_DATE].
            ...
            return list.filter {
                !feedDatabaseManager.doesStreamExist(it) &&
                    ...
                    it.uploadDate!!.offsetDateTime().isAfter(
                        FeedDatabaseManager.Companion.FEED_OLDEST_ALLOWED_DATE
                    )
            }
        }
    }
    ...
    companion object {
        ...
        /**
         * Constant used to check for updates of subscriptions with [NotificationMode.Companion.ENABLED].
         */
