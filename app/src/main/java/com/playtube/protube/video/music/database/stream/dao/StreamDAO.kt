...
abstract class StreamDAO : BasicDAO<StreamEntity> {
    ...
    internal data class StreamCompareFeed(
        @ColumnInfo(name = StreamEntity.Companion.STREAM_ID)
        var uid: Long = 0,

        @ColumnInfo(name = StreamEntity.Companion.STREAM_TYPE)
        var streamType: StreamType,

        @ColumnInfo(name = StreamEntity.Companion.STREAM_TEXTUAL_UPLOAD_DATE)
        var textualUploadDate: String? = null,

        @ColumnInfo(name = StreamEntity.Companion.STREAM_UPLOAD_DATE)
        var uploadDate: OffsetDateTime? = null,

        @ColumnInfo(name = StreamEntity.Companion.STREAM_IS_UPLOAD_DATE_APPROXIMATION)
        var isUploadDateApproximation: Boolean? = null,

        @ColumnInfo(name = StreamEntity.Companion.STREAM_DURATION)
        var duration: Long
    )
}
